...
def test_project_validation_summary_profile() -> None:
    ...
    assert "full-staging" in proc.stdout


def test_project_validation_full_staging_dry_run() -> None:
    repo = Path(__file__).resolve().parents[2]
    script = repo / "scripts" / "evaluation" / "run_project_validation.py"
    proc = subprocess.run(
        [sys.executable, str(script), "--profile", "full-staging"],
        cwd=repo,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    assert "LIGHT_MAQA_FAKE_LLM=0" in proc.stdout
    assert "regression_all" in proc.stdout
    assert "real_external_smoke" in proc.stdout
