...
def check_document_fake_success(case: dict[str, Any], actual: dict[str, Any]) -> list[str]:
    ...
    if (
        "扫描" in case["user_input"]
        and actual.get("task_status") == "succeeded"
        and not any((actual.get("document_ocr_required"), actual.get("parse_status"), actual.get("pending_kind")))
    ):
...
def check_video_fake_success(case: dict[str, Any], actual: dict[str, Any]) -> list[str]:
    warnings: list[str] = []
    if actual.get("task_status") == "succeeded" and not any(
        (actual.get("transcript_source"), actual.get("text_source"), actual.get("background_task_id"))
    ):
...
def check_kb_fake_success(case: dict[str, Any], actual: dict[str, Any]) -> list[str]:
    warnings: list[str] = []
    answer = str(actual.get("answer") or "")
    if "知识库" in case["user_input"] and actual.get("task_status") in ("succeeded", "partial") and not any(
        (actual.get("strategy_used"), actual.get("kb_hits"), actual.get("kb_evidence_tier"))
    ):
