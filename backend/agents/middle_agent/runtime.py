...
- 规则只允许做：
    ...
    * 调 `agno_rag_service / agno_web_service` 真实拉材料（**执行器**，不是主脑）
    * 数值兜底
...
- 当 `mcp_video_ok=True` 时，runtime 自家在 MCP 调用结果就位的同一帧里调
  `rag.video_ingest.ingest_video_bundle(...)`，把文本写进 SQLite FTS 知识库；
...
- 后续问答命中走纯 SQLite FTS（`agno_rag_service.fetch_knowledge_block`），
