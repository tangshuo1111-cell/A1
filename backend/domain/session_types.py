"""
当前会话级历史记忆 —— 三强 Agent 主链共享的"前文承接对象"。
...
- 用结构化锚点（视频入库 source_id）替代"prompt 关键词碰撞"，从源头降低
  被做成假记忆的风险。
...
- 不直接调 LLM；
- 不直接读存储层 —— 我们消费的"上一轮命中"由 service 在每轮产出时回填进 deque
...
class SessionHistorySnapshot:
    ...
    - `prev_kb_sources`：上一轮 service 透传出来的入库 source_id 集合，
       供未来扩展（当前仅用 prev_video 一项作首条联动锚点）。
