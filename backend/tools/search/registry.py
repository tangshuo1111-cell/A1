...
_REGISTRY: dict[str, SearchToolSchema] = {}


def register(schema: SearchToolSchema) -> None:
...
def call_tool(tool_name: str, **kwargs: Any) -> DocumentToolResult:
    ...
    if schema is None:
        return DocumentToolResult(tool_name=tool_name, source_type="web_search", status="failed", error_code="tool_not_found", failure_reason=f"工具 {tool_name} 未注册")
    if not schema.enabled:
        return DocumentToolResult(tool_name=tool_name, source_type="web_search", status="failed", error_code="tool_disabled", failure_reason=f"工具 {tool_name} 已禁用")
