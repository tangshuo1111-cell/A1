"""公开网页视频辅助接口（时长探针，供前端长视频 ASR 确认）。"""

from __future__ import annotations

import time
from concurrent.futures import ThreadPoolExecutor
from concurrent.futures import TimeoutError as FutureTimeoutError

from fastapi import APIRouter, Request
from pydantic import BaseModel, Field

from api.rate_limit import chat_rate_limit_string, limiter
from config.settings import settings
from video.url_fetch import is_supported_video_url, probe_web_video_metadata

router = APIRouter()


class WebVideoMetadataRequest(BaseModel):
    url: str = Field(..., min_length=12, max_length=4000, description="白名单内的视频页 URL（ASCII）")


@router.post("/metadata")
@limiter.limit(chat_rate_limit_string())
def post_web_video_metadata(request: Request, body: WebVideoMetadataRequest) -> dict:
    """yt-dlp 仅取元数据（不下媒体）。白名单外链与 ``fetch_video_text`` 一致。"""
    _ = request
    t0 = time.perf_counter()
    u = (body.url or "").strip()
    if not is_supported_video_url(u):
        return {"ok": False, "error": "url_not_in_whitelist", "latency_ms": int((time.perf_counter() - t0) * 1000)}
    with ThreadPoolExecutor(max_workers=1) as pool:
        fut = pool.submit(probe_web_video_metadata, u)
        try:
            out = fut.result(timeout=float(settings.v16_video_probe_timeout_sec or 12.0))
        except FutureTimeoutError:
            return {
                "ok": False,
                "error": "video_probe_timeout",
                "latency_ms": int((time.perf_counter() - t0) * 1000),
            }
    if isinstance(out, dict):
        out.setdefault("latency_ms", int((time.perf_counter() - t0) * 1000))
    return out
