...
职责：
- 启动专用 daemon worker 线程处理 web_heavy_fetch 任务
- 当 ENABLE_ASYNC_CONTROL_PLANE_V2=True 时，通过统一 async_dispatcher 执行
- 与 task_plane_worker 共享同一 async_task_queue，但专注于 web_heavy_fetch 类型
"""
...
def _worker_loop(worker_index: int) -> None:
    while True:
        ...
        try:
            ...
            logger.warning(
                "web_task_worker failed idx=%s task_id=%s err=%s",
                worker_index,
                message.task_id,
                exc,
            )
...
def ensure_web_task_workers_started() -> None:
    global _started
    if _started:
        return
    with _started_lock:
        ...
        for idx in range(workers):
            thread = threading.Thread(
                ...
                args=(idx,),
                daemon=True,
                name=f"web-task-worker-{idx}",
            )
            thread.start()
        _started = True


def reset_web_task_workers_for_tests() -> None:
    global _started
    with _started_lock:
        _started = False
