...
def apply_answer_fields(out: dict[str, Any], *, answer_type: str | None = None) -> None:
    if answer_type is not None: out["answer_type"] = answer_type

def apply_task_fields(out: dict[str, Any], *, task_id: str | None | object = _UNSET) -> None:
    if task_id is not _UNSET: out["task_id"] = task_id
...
def apply_material_fields(extra: dict[str, Any], envelope: TurnExitEnvelope) -> None:
    if envelope.material_sufficiency is not None: extra["material_sufficiency"] = envelope.material_sufficiency
    if envelope.pending_kind is not None: extra["pending_kind"] = envelope.pending_kind

def apply_timing_fields(out: dict[str, Any], extra: dict[str, Any]) -> None:
    tms = extra.get("timing_total_ms") if extra.get("timing_total_ms") is not None else out.get("workflow_elapsed_ms")
    if tms is not None: extra["timing_total_ms"] = tms
