...
| OCR 调用成功但样本无文本 | **false** | `configured_and_failed` | `ocr_no_text`（如 `FailedOperation.ImageNoText` / `ocr_empty_result`） |
| OCR provider 其他非鉴权失败 | **false** | `configured_and_failed` | `ocr_provider_error` |
