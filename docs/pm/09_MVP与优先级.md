...
- **complex 链**三 Agent 协作：Main → Middle → Answer（**仅复杂任务走此路径**；Fast/Async 不走；经 `application/chat/turn_orchestrator.py` 编排，`run_chat_turn.py` 仅为薄 facade；禁止 Agent 互调）
...
- 默认 CI（`LIGHT_MAQA_FAKE_LLM=1`）：覆盖率 ≥ 60%、unit/migration/contract/smoke + `tests/evaluation/` + `tests/benchmarks/` schema + 架构守卫 + gitleaks；**默认 CI 不跑 42/42 工程回归、7/7 真实外部 smoke**（需 staging 真实 LLM）
- 产品指标 **v1**（`turn_product_metrics` + `aggregate_product_metrics.py`）
- 离线代表题：**42 条** sandbox（`scripts/metrics_sandbox_samples.yaml`：simple×6 / complex×26 / insufficiency×3 / async×3 / reuse×4）
...
- 资料**二次调用率**北极星已接线最小埋点（见 [04](04_产品指标看板.md)）；线上真实流量与硬 SLO 仍待接量，沙箱样本不足时只观测趋势
